...
function f_makeArray(like_array) {
	let type = f_isType(like_array), result_array = [], length = like_array.length;
	if(type === "array") {
		return like_array;
	}
	...
	if(length && type !== "string") {
		while(length--) {
			result_array.push(like_array[length]);
		}
	}
